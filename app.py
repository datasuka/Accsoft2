...
# --- generate voucher ---
def buat_voucher(df, no_voucher, settings, pejabat):
    pdf = FPDF("P", "mm", "A4")
    ...
    # Header kanan (judul dokumen rapih dalam garis)
    ...
    pdf.cell(80, 8, judul, border="TB", align="C", ln=1)
...
st.markdown("""
Aplikasi ini membantu menghasilkan **bukti transaksi akuntansi** dalam bentuk PDF 
(Jurnal Voucher / Kas / Bank) dari file Excel.  
⚠️ *Disclaimer*: Aplikasi ini berjalan lokal, **tidak menyimpan data Anda ke web**.  
...
2. Pastikan setiap voucher memiliki **Debit dan Kredit seimbang**.
3. Upload file Excel ke aplikasi ini.
4. Pilih mode cetak:
   - **Single Voucher** → cetak satu voucher.
   - **Per Bulan** → cetak semua voucher dalam 1 bulan ke ZIP.
5. Download hasilnya dalam bentuk PDF/ZIP.
""")
...
if file:
    ...
    if mode == "Single Voucher":
        ...
        if st.button("Cetak"):
            pdf_file = buat_voucher(df, no_voucher, settings, pejabat)
        ...
        if st.button("Cetak Semua Voucher Bulan Ini"):
            ...
            with zipfile.ZipFile(buffer_zip, "w") as zf:
                for v in df[df["Tanggal"].dt.month==bulan]["Nomor Voucher Jurnal"].unique():
                    pdf_file = buat_voucher(df, v, settings, pejabat)
